...
class MiddlewareService<T: TargetType & AccessTokenAuthorizable>: MiddlewareProtocol {
    ...
    let provider: MoyaProvider<T> = {
        let endpointClosure = { (target: T) -> Endpoint in
            ...
            defaultEndpoint = defaultEndpoint.adding(
                newHTTPHeaderFields: [
                    HTTPHeaderField.contentLanguage.rawValue: "en",
     ...
     func request<R: Codable>(target: T) -> AnyPublisher<R,MiddlewareError> {
         ...
         provider.request(target) {
             if let response = try? $0.get() {
                 let url = "\(response.request?.httpMethod ?? "") \(response.request?.description ?? "")"
                 let request = response.data.toString()
             }
             switch $0 {
...
extension AccessTokenAuthorizable {
    ...
    var authentications: [String: String] {
        ...
        case .basic: return [:]
        case .bearer:
            let autService = Authentications.shared
            return ["Authorization": "Bearer \(autService.fetchToken)"]
        }
    }
}
